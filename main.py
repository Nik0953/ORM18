...
# создаем базу данных, только если она еще не создана
import os.path

if not os.path.exists('data/new_data/new_hh.db'):
    from make_new_db import *
...
def run():
    ...
    if request.method == 'POST':
        ...
        req_form = {}
        req_form['where_to_find'] = int(request.form['where_to_find'])
        ...
        req_web = {}
        req_web['where_to_find'] = [s for s in reg if int(reg[s])==req_form['where_to_find']][0]
        req_web['what_skills'] = [s for s in sk if int(sk[s])==req_form['what_skills']][0]
        req_web['sal_min'] = req_form['sal_min']
        print(req_web)

        v_lst = get_vac_info(where_to_find=req_form['where_to_find'],
